(async () => {
  const coinsEl = document.getElementById("studentMySetsCoins");
  const avatarEl = document.getElementById("studentMySetsAvatar");
  const nameEl = document.getElementById("studentMySetsName");
  const statusEl = document.getElementById("studentMySetsStatus");
  const roomCodeEl = document.getElementById("studentMySetsRoomCode");
  const metaEl = document.getElementById("studentMySetsMeta");
  const contentEl = document.getElementById("studentMySetsContent");
  ...
  function mySets() {
    const uploads = (Array.isArray(state.quizzes) ? state.quizzes : []).filter((set) => String(set?.source || "") !== "built_in");
    if (!state.student) {
      return [];
    }
    const displayName = String(state.student.displayName || "").trim().toLowerCase();
    const username = String(state.student.username || "").trim().toLowerCase();
    return uploads.filter((set) => {
      const uploadedBy = String(set?.uploadedBy || "").trim().toLowerCase();
      return uploadedBy && (uploadedBy === displayName || uploadedBy === username);
    });
  }
  ...
  function renderHeader() {
    ...
    if (statusEl) {
      statusEl.textContent = state.activeRoom?.code ? `Room ${state.activeRoom.code}` : "My saved sets";
    }
    if (roomCodeEl) {
  ...
  function renderMeta(sets) {
    ...
    const pills = [
      `${formatCount(sets.length)} custom set${sets.length === 1 ? "" : "s"}`,
      state.student ? `Signed in as ${studentName()}` : "Sign in to load your sets",
    ...
    contentEl.innerHTML = `
      <div class="empty-shell">
        <div class="empty-card">
          <h2>You'll Need a Question Set to Host!</h2>
          <div class="empty-actions">
            <a class="action purple" href="/student-set-creator">Create a Set</a>
            <a class="action teal" href="/student-discover">Discover Sets</a>
          </div>
          <a class="tutorial" href="/student-set-creator">Getting Started Tutorial</a>
        </div>
      </div>
    `;
  }
        ...
        ${sets
          .map((set) => {
            ...
            const hero = set.coverImage
              ...
              : `<div class="card-hero" style="background:linear-gradient(135deg,${escapeHtml(start)} 0%,${escapeHtml(end)} 100%)">${escapeHtml((set.creationMethod || "manual").toUpperCase())}</div>`;
            const description = String(set.description || "").trim() || "Custom question set saved on this server.";
            return `
              <article class="card">
                ${hero}
                <div class="card-body">
                  <h2 class="card-title">${escapeHtml(set.label || "Custom Set")}</h2>
                  <p class="card-copy">${escapeHtml(description)}</p>
                  <div class="chip-row">
                    <span class="chip">${escapeHtml(`${formatCount(set.questionCount || 0)} Questions`)}</span>
                    <span class="chip">${escapeHtml(set.visibility || "public")}</span>
                    <span class="chip">${escapeHtml(set.category || "Mixed")}</span>
                  </div>
                  <div class="card-actions">
                    <a class="action purple" href="/student-set-creator?set=${encodeURIComponent(set.id || "")}">Edit Set</a>
                    <a class="ghost" href="/student-discover">Discover Sets</a>
                  </div>
                </div>
              </article>
            `;
